...
def get_datasets():
    ...
    df = pd.read_csv('data1/last_val_mae.csv')
    df.sort_values(by='mae')

    df_paper = pd.read_csv('data1/paper_mae_raw.csv') # pdf text -> csv
    ...
    df_paper.to_csv('data1/paper_mae.csv', index=False)
    ...
    df_paper = df_paper.reset_index(drop=True)
    # save as csv
    df_paper.to_csv('data1/paper_mae_normalized.csv', index=False)
    return benchmarks
